...
from tensorflow.keras.layers import Input, Embedding, Masking, LSTM, Dense, Activation, Permute, Multiply, Lambda, RepeatVector, Flatten
...
lstm_output = LSTM(64, return_sequences=True)(mask)
...
joblib.dump(history.history, "lstm_attention_history.pkl")
...
results_df.to_csv("lstm_attention_metrics.csv", index=False)
...
plt.title("График функции потерь (MSE) по epochs (LSTM)")
plt.xlabel("epochs")
plt.ylabel("MSE")
plt.legend()
plt.grid(True)
...
plt.title("Средняя абсолютная ошибка (MAE) по epochs (LSTM)")
plt.xlabel("epochs")
plt.ylabel("MAE")
plt.legend()
plt.grid(True)
...
plt.title("Фактические vs. Предсказанные значения (LSTM)")
plt.grid(True)
plt.axis('equal')
plt.show()
...
plt.title("Распределение остатков (фактическое - предсказанное) (LSTM)")
plt.xlabel("Ошибка")
...
plt.title("Attention-веса (LSTM модель)")
plt.show()
